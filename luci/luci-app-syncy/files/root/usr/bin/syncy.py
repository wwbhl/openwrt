...
class SyncY():
    ...
    def __init__(self, argv=sys.argv[1:]):
        ...
        if 'refresh_token' not in SyncY.syncytoken or SyncY.syncytoken['refresh_token'] == '' or (len(self.__argv) != 0 and self.__argv[0] in ["sybind", "cpbind"]):
            ...
            if (('device_code' not in SyncY.syncytoken or SyncY.syncytoken['device_code'] == '') and len(self.__argv) == 0) or (len(self.__argv) != 0 and self.__argv[0] == "sybind"):
                http_code, response_str = sycurl.request('https://syncyhome.duapp.com/syserver', 'method=bind_device&scope=basic,netdisk', 'POST', SYCurl.Normal)
                if http_code != 200:
            ...
            if len(self.__argv) != 0 and self.__argv[0] == "cpbind":
                ...
                if m:
                    SyncY.syncytoken['device_code'] = m[0]
                    m = re.findall(r'.*\"time\":([0-9]+).*', bindinfo)
                    if int(time.time()) - int(m[0]) >= 1800:
                        sys.exit(1)
                else:
                    sys.exit(1)
            http_code, response_str = sycurl.request('https://syncyhome.duapp.com/syserver', 'method=get_device_token&code=%s' % (SyncY.syncytoken['device_code']), 'POST', SYCurl.Normal)
            ...
            if m:
                SyncY.syncytoken['refresh_token'] = m[0]
                m = re.findall(r'.*\"access_token\":\"([^"]+)\".*', response_str)
                SyncY.syncytoken['access_token'] = m[0]
                m = re.findall(r'.*\"expires_in\":([0-9]+).*', response_str)
                SyncY.syncytoken['expires_in'] = int(m[0])
    ...
    def __check_expires(self):
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://openapi.baidu.com/rest/2.0/passport/users/getLoggedInUser', 'access_token=%s' % SyncY.syncytoken['access_token'], 'POST', SYCurl.Normal)
        m = re.findall(r'.*\"uid\":\"([0-9]+)\".*', response_str)
        if m:
            http_code, response_str = sycurl.request('https://syncyhome.duapp.com/syserver', 'method=get_last_version&edition=python&ver=%s&uid=%s' % (__VERSION__, m[0]), 'POST', SYCurl.Normal)
            if http_code == 200 and response_str.find('#') > -1:
                ...
                if lastver != __VERSION__:
                    ...
                    self.printlog('%s %s' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), smessage))
        if (SyncY.syncytoken['refresh_date'] + SyncY.syncytoken['expires_in'] - 864000) > int(time.time()):
            return
        http_code, response_str = sycurl.request('https://syncyhome.duapp.com/syserver', 'method=refresh_access_token&refresh_token=%s' % (SyncY.syncytoken['refresh_token']), 'POST', SYCurl.Normal)
        if http_code != 200:
            self.writeerror('%s ERROR: Refresh access token failed: %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), response_str))
            return 1
        m = re.findall(r'.*\"refresh_token\":\"([^"]+)\".*', response_str)
        if m:
            SyncY.syncytoken['refresh_token'] = m[0]
            ...
            SyncY.syncytoken['refresh_date'] = int(time.time())
            self.__save_config()
            self.printlog(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' Refresh access token success.')
            return 0
        else:
            self.writeerror('%s ERROR: Refresh access token failed: %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), response_str))
            return 1
    ...
    def __check_pcspath(pcsdirname, pcsfilename):
        if len(pcsdirname) + len(pcsfilename) + 1 >= 1000:
            SyncY.writeerror('%s ERROR: Length of PCS path(%s/%s) must less than 1000, skip upload.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), pcsdirname, pcsfilename))
            return 1
        if SyncY.syre['pcspath'].findall(pcsfilename):
            SyncY.writeerror('%s ERROR: PCS path(%s/%s) is invalid, please check whether special characters exists in the path, skip upload the file.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), pcsdirname, pcsfilename))
            return 1
        return 0
    ...
    def __get_pcs_quota():
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/quota?method=info&access_token=%s' % (SyncY.syncytoken['access_token']), '', 'GET', SYCurl.Normal)
        if http_code != 200:
            SyncY.writeerror('%s ERROR: Get pcs quota failed(error code:%d),%s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, response_str))
            return 1
        quota = json.loads(response_str)
        if 'quota' in quota:
            SyncY.printlog('%s PCS quota is %dG,used %dG.' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), quota['quota'] / 1024 / 1024 / 1024, quota['used'] / 1024 / 1024 / 1024))
            return 0
        else:
            SyncY.writeerror('%s ERROR: Get pcs quota failed,%s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), response_str))
            return 1
    ...
    def __get_pcs_filelist(pcspath, startindex, endindex):
        ...
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=list&access_token=%s&path=%s&limit=%d-%d&by=name&order=asc' % (SyncY.syncytoken['access_token'], uripath, startindex, endindex), '', 'GET', SYCurl.Normal)
        rfnlist = json.loads(response_str)
        try:
            if http_code != 200:
                if 'error_code' in rfnlist and rfnlist['error_code'] == 31066:
                    return 31066, []
                else:
                    SyncY.writeerror('%s ERROR: Get file list failed(error code:%d): %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, pcspath, response_str))
                    return 1, []
            if 'list' in rfnlist:
                return 0, rfnlist['list']
            else:
                SyncY.writeerror('%s ERROR: Get file list failed(code:%d): %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, pcspath, response_str))
                return 1, []
        finally:
            del rfnlist
    ...
    def __rm_pcsfile(self, pcspath, slient=False):
        uripath = quote_plus(pcspath)
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=delete&access_token=%s&path=%s' % (SyncY.syncytoken['access_token'], uripath), '', 'POST', SYCurl.Normal)
        if http_code != 200:
            if not slient:
                self.writeerror('%s ERROR: Delete remote file or directory "%s" failed(error code:%d): %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), pcspath, http_code, response_str))
            return 1
        if not slient:
    ...
    def __mv_pcsfile(self, oldpcspath, newpcspath, slient=False):
        ...
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=move&access_token=%s&from=%s&to=%s' % (SyncY.syncytoken['access_token'], uripaths, uripathd), '', 'POST', SYCurl.Normal)
        if http_code != 200:
            if not slient:
                self.writeerror('%s ERROR: Move remote file or directory "%s" to "%s" failed(error code:%d): %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), oldpcspath, newpcspath, http_code, response_str))
            return 1
        if not slient:
    ...
    def __cp_pcsfile(self, srcpcspath, destpcspath):
        ...
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=copy&access_token=%s&from=%s&to=%s' % (SyncY.syncytoken['access_token'], uripaths, uripathd), '', 'POST', SYCurl.Normal)
        if http_code != 200:
            self.writeerror('%s ERROR: Copy remote file or directory "%s" to "%s" failed(error code:%d): %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), srcpcspath, destpcspath, http_code, response_str))
            return 1
    ...
    @staticmethod
    def __get_pcs_filemeta(filepath):
        uripath = quote_plus(filepath)
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=meta&access_token=%s&path=%s' % (SyncY.syncytoken['access_token'], uripath), '', 'GET', SYCurl.Normal)
        if http_code != 200:
            SyncY.writeerror('%s ERROR: Get file meta failed(error code:%d): %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, filepath, response_str))
            return 1, {}
    ...
    def __upload_file_nosync(self, filepath, pcspath):
        uripath = quote_plus(pcspath)
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://c.pcs.baidu.com/rest/2.0/pcs/file?method=upload&access_token=%s&path=%s&ondup=newcopy' % (SyncY.syncytoken['access_token'], uripath), '0-%d' % (os.stat(filepath).st_size - 1), 'POST', SYCurl.Upload, filepath)
        if http_code != 200:
            self.writeerror('%s ERROR: Upload file to pcs failed(error code:%d): %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, filepath, response_str))
            return 1
    ...
    def request(self, url, rdata='', method='POST', rtype=0, fnname=''):
        ...
        while retrycnt <= SyncY.config['retrytimes']:
            ...
            try:
                ...
                if http_code < 400 or http_code == 404 or retrycnt == SyncY.config['retrytimes']:
                    return http_code, self.__response_str
                else:
                    retrycnt += 1
                ...
                if retrycnt == SyncY.config['retrytimes']:
                    return errno, self.__response_str
                else:
                    retrycnt += 1
            finally:
                curl.close()
...
class SYTask(threading.Thread):
    ...
    def run(self):
        try:
            ...
            if self.__op == SYCurl.Upload:
                ...
                SyncY.writeerror('%s Unknown sync operation(%s) of threading operation.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self.__op))
            ...
            SyncY.writeerror('%s Transfer task exception error occurred: %s .\n%s\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), e, traceback.format_exc()))
            SyncY.failcount_increase()
        finally:
    ...
    def __upload_file(self):
        ...
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://c.pcs.baidu.com/rest/2.0/pcs/file?method=upload&access_token=%s&path=%s&ondup=%s' % (SyncY.syncytoken['access_token'], uripath, self.__ondup), '0-%d' % (os.stat(self.__filepath).st_size - 1), 'POST', SYCurl.Upload, self.__filepath)
        if http_code != 200:
            SyncY.writeerror('%s ERROR: Upload file to pcs failed(error code:%d): %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, self.__filepath, response_str))
            return 1
        responses = json.loads(response_str)
        if 'size' in responses and responses['size'] == self.__fsize:
            self.__rmd5 = responses['md5']
        else:
            SyncY.writeerror('%s ERROR: Upload File failed, remote file error : %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self.__filepath, response_str))
            sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=delete&access_token=%s&path=%s' % (SyncY.syncytoken['access_token'], uripath), '', 'POST', SYCurl.Normal)
            return 1
        self.__save_data()
    ...
    def __rapid_uploadfile(self):
        ...
        uripath = quote_plus(self.__pcspath)
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=rapidupload&access_token=%s&path=%s&content-length=%d&content-md5=%s&slice-md5=%s&content-crc32=%s&ondup=%s' % (SyncY.syncytoken['access_token'], uripath, self.__fsize, contentmd5, slicemd5, crc, self.__ondup), '', 'POST', SYCurl.Normal)
        responses = json.loads(response_str)
        if http_code != 200:
            if 'error_code' in responses and responses['error_code'] == 31079:
                ...
                SyncY.writeerror('%s ERROR: Rapid upload file failed(error code:%d): %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, self.__filepath, response_str))
                return 1
        else:
            if 'size' in responses and responses['size'] == self.__fsize:
                self.__rmd5 = responses['md5']
            else:
                SyncY.writeerror('%s ERROR: File is rapid uploaded, but can not get remote file size: %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self.__filepath, response_str))
                return 1
            self.__save_data()
    ...
    def __slice_uploadfile(self):
        ...
        uripath = quote_plus(self.__pcspath)
        sycurl = SYCurl()
        http_code, response_str = sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=createsuperfile&access_token=%s&path=%s&ondup=%s' % (SyncY.syncytoken['access_token'], uripath, self.__ondup), 'param=%s' % json.dumps(param), 'POST', SYCurl.Normal)
        if http_code != 200:
            SyncY.writeerror('%s ERROR: Create superfile failed(error code:%d): %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, self.__filepath, response_str))
            return 1
        ...
        if 'size' in responses and responses['size'] == self.__fsize:
            ...
            SyncY.writeerror('%s ERROR: Slice upload file failed: %s, %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self.__filepath, response_str))
            sycurl.request('https://pcs.baidu.com/rest/2.0/pcs/file?method=delete&access_token=%s&path=%s' % (SyncY.syncytoken['access_token'], uripath), '', 'POST', SYCurl.Normal)
            return 1
        self.__save_data()
    ...
    def __download_file(self):
        ...
        if self.__rsize <= (SyncY.config['blocksize'] + 1) * 1048576:
            sycurl = SYCurl()
            http_code = sycurl.request('https://d.pcs.baidu.com/rest/2.0/pcs/file?method=download&access_token=%s&path=%s' % (SyncY.syncytoken['access_token'], uripath), '0-%d' % (self.__rsize - 1), 'GET', SYCurl.Download, '%s.syy' % self.__filepath)[0]
            if http_code != 200 and http_code != 206:
                SyncY.writeerror('%s ERROR(%d): Download file(%s) failed.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, self.__pcspath))
                return 1
        else:
            with open('%s.db.syy' % self.__filepath, 'r') as dlfn:
        ...
        if fmeta.st_size != self.__rsize:
            SyncY.writeerror('%s ERROR: Download file failed: "%s", downloaded file size not equal to remote file size.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self.__pcspath))
        ...
        try:
            ...
            while True:
                ...
                if SyncY.synctask[self.__fnmd5][0][0] == 'upload':
                    http_code, response_str = sycurl.request('https://c.pcs.baidu.com/rest/2.0/pcs/file?method=upload&access_token=%s&type=tmpfile' % (SyncY.syncytoken['access_token']), '%d-%d' % (startpos, endpos), 'POST', SYCurl.Upload, self.__filepath)
                    if http_code != 200:
                        SyncY.writeerror('%s ERROR(%d): Slice upload file(%s) failed: %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, self.__filepath, response_str))
                        return 1
                    responses = json.loads(response_str)
                    if 'md5' in responses:
                        if self.__threadcond.acquire():
                        ...
                        SyncY.writeerror('%s ERROR: Upload File(%s) failed, remote file error : %s.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), self.__filepath, response_str))
                        return 1
                elif SyncY.synctask[self.__fnmd5][0][0] == 'download':
                    uripath = quote_plus(self.__pcspath)
                    http_code = sycurl.request('https://d.pcs.baidu.com/rest/2.0/pcs/file?method=download&access_token=%s&path=%s' % (SyncY.syncytoken['access_token'], uripath), '%d-%d' % (startpos, endpos), 'GET', SYCurl.Download, '%s.syy' % self.__filepath)[0]
                    if http_code != 200 and http_code != 206:
                        SyncY.writeerror('%s ERROR(%d): Slice download file(%s) failed.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), http_code, self.__pcspath))
                        return 1
                    if self.__threadcond.acquire():
                        SyncY.synctask[self.__fnmd5][idx][2] = 1
                        idx, startpos, endpos = self.__get_nextslice()
                    ...
                    SyncY.writeerror('%s Unknown operation(%s) of threading operation.\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), SyncY.synctask[self.__fnmd5][0][0]))
                    return 1
        except Exception, e:
            SyncY.writeerror('%s Transfer thread exception error occurred, error: %s .\n%s\n' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), e, traceback.format_exc()))
        finally:
            if self.__threadcond.acquire():
                if idx != 0:
